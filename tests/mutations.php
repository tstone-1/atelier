...
$mutations = array(
	...
	array(
		'id'      => 'C6',
		'file'    => $config,
		'find'    => "\t\treturn trim( wp_strip_all_tags( (string) \$css ) );",
		'replace' => "\t\treturn trim( (string) \$css );",
		'expect'  => 'custom css cannot close its style element',
		'why'     => 'the value is printed inside a style element',
	),
	array(
		'id'      => 'C7',
		'file'    => $config,
		'find'    => "\t\treturn trim( wp_strip_all_tags( (string) \$css ) );",
		'replace' => "\t\treturn trim( str_replace( '>', '', wp_strip_all_tags( (string) \$css ) ) );",
		'expect'  => 'custom css keeps the child combinator',
		'why'     => 'stripping > would silently rewrite ordinary selectors',
	),
	array(
		'id'      => 'I1',
		'file'    => $item,
	...
	array(
		'id'      => 'B7',
		'file'    => $config,
		'find'    => "\t\t\$css = preg_replace( '/#envira-gallery-wrap-(\\d+)/', '#atelier-\$1-wrap', \$css );\n\n\t\treturn str_replace( '#envira-gallery-', '#atelier-', \$css );",
		'replace' => "\t\treturn \$css;",
		'expect'  => 'custom css is rewritten to atelier ids',
		'why'     => 'sixteen galleries carry hand-written css keyed on envira\'s element ids',
	),
	array(
		'id'      => 'B7b',
		'file'    => $config,
		'find'    => "\t\t\$css = preg_replace( '/#envira-gallery-wrap-(\\d+)/', '#atelier-\$1-wrap', \$css );\n\n\t\treturn str_replace( '#envira-gallery-', '#atelier-', \$css );",
		'replace' => "\t\t\$css = preg_replace( '/#envira-gallery-wrap-(\\d+)/', '#atelier-wrap-\$1', \$css );\n\n\t\treturn str_replace( '#envira-gallery-', '#atelier-', \$css );",
		'expect'  => 'rewritten css selectors match ids that are rendered',
		'why'     => 'the wrap form shipped for months: the rewrite said #atelier-wrap-N while the renderer emitted id="atelier-N-wrap", so every converted wrapper rule matched nothing. Neither half could fail alone -- each was pinned against the code that produced it.',
	),
	array(
		'id'      => 'SLUG1',
